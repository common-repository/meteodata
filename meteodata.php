...
add_action( 'admin_menu', 'MeteoData_menu' );

/** Step 1. */
function MeteoData_menu() {
	add_options_page( 'MeteoData Options', 'MeteoData', 'manage_options', 'MeteoData', 'MeteoData_options' );
...
function register_mysettings() {
	...
	register_setting( 'MeteoData-group', 'pad' );
	register_setting( 'MeteoData-group', 'datumtijdkoppel' );
	register_setting( 'MeteoData-group', 'spatieren' );
}

/** Step 3. */
function MeteoData_options() {
	...
	?>
		<div class="wrap">
			<h2><?php _e('Meteohub settings'); ?></h2>
				...
				<?php settings_fields( 'MeteoData-group' ); ?>
				<?php do_settings_sections( 'MeteoData-group' ); ?>
...
function meteodata_func( $atts ) {
	...
	if(strstr($sensor, '_trend')){
		$path = '/wp-content/plugins/meteodata';
		if($output == 1)
...
add_shortcode( 'meteodata', 'meteodata_func' );